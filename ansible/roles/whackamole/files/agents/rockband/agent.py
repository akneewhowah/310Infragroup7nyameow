    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=SERVER_TIMEOUT, context=CTX) as response:
            if response.getcode() == 200:
                ...
                if endpoint == "agent/beacon/rockband":
...
class AuthWatcher:
    ...
    def evaluate_threat(self, auth):
        ...
        if msg:
            ...
            send_message("agent/beacon/rockband", old_status, new_status, msg, authInfo=auth)
            return True
        return False
...
def main(stop_event=None):
    ...
    send_message("agent/beacon/rockband",True,True,f"Register")
    ...
    while True:
        ...
        if PAUSED != pausedStatus:
            ...
            if PAUSED:
                ...
                send_message("agent/beacon/rockband",False,False,f"Agent moved into PAUSE status for {int(pausedEpochLocal - time.time())} seconds")
            else:
                send_message("agent/beacon/rockband",True,True,f"Agent moved into ACTIVE status (from PAUSE)")
        if not PAUSED:
            watcher.config = watcher.fetch_config()
            sent_msg = watcher.analyze_log()
            if not sent_msg:
                send_message("agent/beacon/rockband",True,True,"all good")
            print_debug(f"main(): sleeping for {SLEEPTIME} seconds")
            print_debug(f"")
        else:
            if not suppressed_send:
                send_message("agent/beacon/rockband",True,False,f"Agent still in PAUSE status for {int(pausedEpochLocal - time.time())} seconds remaining")
