    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=SERVER_TIMEOUT, context=CTX) as response:
            if response.getcode() == 200:
                ...
                if endpoint == "agent/beacon/paperking":
...
def firewall_rules_create_windows(port,direction,action):
    rule_name = f"paperking_Rule_{port}_{direction}_{action}"
    ps_cmd = fr"""
    ...
    """
    if DISARM:
        return False, [f"DISARMED, but told to create firewall rule paperking_Rule_{port}_{direction}_{action}"] 
    if run_powershell(ps_cmd):
        return True, [f"SUCCESSFULLY created firewall rule paperking_Rule_{port}_{direction}_{action}"]
    else:
        return False, [f"FAILED to create firewall rule paperking_Rule_{port}_{direction}_{action}"]
...
def pause(seconds=60):
    send_message("agent/beacon/paperking",True,True,f"pausing for seconds {seconds}")
    return True
def resume(scheduled=False):
    send_message("agent/beacon/paperking",True,True,f"resuming - scheduled: {scheduled}")
    return True
def reregister():
    send_message("agent/beacon/paperking",True,True,"reregister")
    return True
...
def main(stop_event=None):
    ...
    send_message("agent/beacon/paperking",True,True,f"Register")
    ...
    while True:
        ...
        if PAUSED != pausedStatus:
            ...
            if PAUSED:
                ...
                send_message("agent/beacon/paperking",False,False,f"Agent moved into PAUSE status for {int(pausedEpochLocal - time.time())} seconds")
            else:
                send_message("agent/beacon/paperking",True,True,f"Agent moved into ACTIVE status (from PAUSE)")
        if not PAUSED:
            ...
            for issue in result_issues:
                ...
                if newIssues[-1] not in oldIssues:
                    send_message("agent/beacon/paperking",result_oldStatus,result_newStatus,newIssues[-1])
                    sent_msg = True
                else:
                    suppressed_send = True
            ...
            if not sent_msg:
                if suppressed_send:
                    send_message("agent/beacon/paperking",True,True,"no new issues; at least one prior issue still exists but suppressing redundant alert")
                else:
                    send_message("agent/beacon/paperking",True,True,"all good")
            ...
            if not suppressed_send:
                send_message("agent/beacon/paperking",True,False,f"Agent still in PAUSE status for {int(pausedEpochLocal - time.time())} seconds remaining")
        """
        system = platform.system()
        if system == "Windows":
            if len(sys.argv) > 1:
